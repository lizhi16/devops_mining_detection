...
#define FILE_WRITE "/home/test/targetAddrs.list"
...
//lizhi: origin 1000 * 1000 change to 1000 * 1000 * 1000
static unsigned long aggr_interval = 1000 * 1000 * 100;
static unsigned long regions_update_interval = 1000 * 1000 * 100;

//lizhi: time window
static unsigned long time_windows = 1000 * 1000 * 1000;
		...
		struct region regions[3])
{
	...
	for (; vma; vma = vma->vm_next) {
		...
		if (prev_vma == NULL) {
			...
			if (i == 1000)
			{
				printk("[LIZHI] this is original start... %lx\n", vma->vm_start);
				start = vma->vm_start;
			}*/
			start = vma->vm_start;
			//start = vma->vm_start;
	...
	printk("[LIZHI] regions[0].start-regions[0].end: %lx-%lx\n", regions[0].start, regions[0].end);
	printk("[LIZHI] regions[1].start-regions[1].end: %lx-%lx\n", regions[1].start, regions[1].end);
	printk("[LIZHI] regions[2].start-regions[2].end: %lx-%lx\n", regions[2].start, regions[2].end);
}
...
static void aggregate(void)
{
	...
	mapia_for_each_task(t) {
		...
		mapia_for_each_region(r, t) {
			...
			//lizhi
			int leng = r->vm_end-r->vm_start;
			if (r->nr_accesses > 0)
			{
				all_access += r->nr_accesses;
				printk("[LIZHI] (%d) %lx - %lx (%lx): %d\n", leng, r->vm_start, r->vm_end, r->sampling_addr, r->nr_accesses);
			}
...
static int mapia_trace_thread(void *data)
{
	...
	//while (i <= 20) {
		...
		if (need_aggregate()) {
			...
			//lizhi: time window
			udelay(regions_update_interval * 10);
	...
	int k;
	for (k=0;k<10;k++)
		printk("[LIZHI] %lx: %d\n", targets_addr[k], nr_targets_addr[k]/index);
